...
export function isVirtualVueFile(path: string, projectFiles: Set<string>) {
  return path.endsWith('.san.ts') && (!path.includes('node_modules') || projectFiles.has(path.slice(0, -'.ts'.length)));
}
...
export function isVirtualVueTemplateFile(path: string) {
  return path.endsWith('.san.template');
}
