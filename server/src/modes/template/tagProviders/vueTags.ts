...
const vueDirectives = [
  ...
  genAttribute('s-html', undefined, 'Updates the element’s `innerHTML`. XSS prone.'),
  genAttribute('s-if', undefined, 'Conditionally renders the element based on the truthy-ness of the expression value.'),
  genAttribute('s-else', 'v', 'Denotes the “else block” for `s-if` or a `s-if`/`s-else-if` chain.'),
  genAttribute('s-elif', undefined, 'Denotes the “else if block” for `s-if`. Can be chained.'),
  genAttribute('s-for', undefined, 'Renders the element or template block multiple times based on the source data.'),
  genAttribute(
    'slot',
    undefined,
