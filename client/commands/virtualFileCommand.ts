...
export function generateShowVirtualFileCommand(client: LanguageClient) {
  return async () => {
    if (!vscode.window.activeTextEditor || !vscode.window.activeTextEditor.document.fileName.endsWith('.san')) {
      return vscode.window.showInformationMessage(
        'Failed to show virtual file. Make sure the current file is a .san file.'
      );
    }
...
[SanFileStart, SanFileEnd, SanFileText] => [TSVirtualFileStart, TSVirtualFileEnd, TSVirtualFileText]
${separator}
